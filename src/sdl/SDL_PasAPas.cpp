...
void sdlJeuPasAPas::sdlAffChrono(int x, int y, int largeur, int hauteur) {
    jeu.chrono.update();// a enlever apres les test, ne doit pa �tre la
    SDL_Color couleur = { 0, 0, 0 };
    SDL_Surface* texte = nullptr;
    SDL_Rect position;
    SDL_Texture* texte_texture = NULL;    //Create Texture pointer
...
void sdlJeuPasAPas::sdlBoucle() {
    ...
    while (gameRunning) {
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            //----On fait les actions li� au differentes touches du clavier si elles sont press�es
            ...
            //----On fait les actions li� aux clics souris
