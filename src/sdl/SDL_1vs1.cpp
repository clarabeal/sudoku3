...
sdl1Vs1::sdl1Vs1(unsigned char d) : jeu(d), gestionnaireSauvegarde("data/saves/", "../data/saves/"), dimGrille(d), autoSave(false){
    ...
    window = SDL_CreateWindow("Sudoku 3 | Mode 1 Vs 1", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WIDTH, HEIGHT, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
    if (window == nullptr)
    {
...
void sdl1Vs1::init_hit_menu(int x1, int y1, int x2, int y2) {
    ...
    if (!autoSave && jeu.sauvegardeId != 0) { // si la sauvegarde automatique est desactivée et que la partie a deja ete sauvegardé au moins une fois
        ...
        tabHitBoxSelectionMenu[8].x1 = x1;  //on place les hitboxe des btn pour activer la sauvegarde auto (2hitbox une pour l'image avec la souris dessus et une pour limage quand la souris n'est pas la)
...
void sdl1Vs1::resetTabHitSelectionMenu(){
...
void sdl1Vs1::sdlAffGrille(Grille& grille, int x, int y, int largeur, int hauteur, bool force) {

    im_grille.draw(renderer, x, y, largeur, hauteur);

    if (!ChronoJactif->estEnPause() || force) {
...
void sdl1Vs1::sdlAffChrono(int x, int y, int largeur, int hauteur, chronometre &chrono, bool full) {
    ...
    if (full) {
        buffConversion[sprintf(buffConversion, "%luh  %lum  %lus %lums", chrono.getTimeInHours(), chrono.getTimeInMin() % 60, chrono.getTimeInSec() % 60, chrono.getTimeInMSec()%1000) + 1] = '\0';

    }
    else {
        buffConversion[sprintf(buffConversion, "%luh  %lum  %lus", chrono.getTimeInHours(), chrono.getTimeInMin() % 60, chrono.getTimeInSec() % 60) + 1] = '\0';
